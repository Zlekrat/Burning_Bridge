...
def pretty_print(text, line_length=50):
    ...
    for line in text:
        if line_len > line_length:
            #print(" ".join(word_list).strip().replace("\n"," "))
            word_list.clear()
            line_len = 0
        else:
...
if __name__ == '__main__':
    if len(sys.argv)!=3:
        ...
        if sys.argv[1]=='-t':
            ...
            result, avg_truth = truth_logic(text, articles, website=False)
            print(", ".join(extract_keywords(text)))
            sys.exit(int(avg_truth*100))

        elif sys.argv[1]=='-s':
            #print(sys.argv[2])
            article, articles, global_ranks = parse_url(sys.argv[2])
            from data_analysis import truth_logic
            result, avg_truth = truth_logic(article, articles)
            #print('Average truth coeff: ', avg_truth)
            #print('FINAL RESULT: ' + str(result))
            print(str(authors_p))
            print(str(date_p))
            print(", ".join(keywords_p))
            print(", ".join([country[0]+':'+str(country[1]) for country in global_ranks]))
            save_graph(global_ranks)
            sys.exit(int(avg_truth*100))
        else:
            print('Wrong arguments (-t file.txt, -s website.html)')
    #pretty_print(text, line_length=50)
